...
export const Bleachers = () => {
  ...
  const segments = useMemo(() => {
    const segs = [];
    const angleStep = (Math.PI / 1.5) / 20; // Spread over 120 degrees
  ...
  return (
    <group position={[0, 0, 0]}>
      {/* Main Structure - simplified as blocks for now */}
      {segments.map((seg, i) => (
          <mesh 
            key={i} 
          ...
          >
            ...
            <meshStandardMaterial color={seg.level % 2 === 0 ? "#334155" : "#475569"} roughness={0.8} />
          </mesh>
      ))}
      
      {/* Side Walls/Railings */}
       <mesh position={[20, 2.5, 20]} rotation={[0, -Math.PI/4, 0]}>
